...
const { initialized, isAuthenticated, isAdmin } = storeToRefs(authStore)
</script>
...
<template>
  ...
  <RouterView v-else-if="isAuthenticated && isAdmin" />
  ...
  <div v-else class="max-w-3xl mx-auto px-4 py-12">
    <div class="rounded-3xl border border-red-200 bg-white p-8 text-center shadow-sm">
      <p class="mb-3 text-xs font-semibold uppercase tracking-[0.3em] text-red-700">
        Доступ запрещён
      </p>
